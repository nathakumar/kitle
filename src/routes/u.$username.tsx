...
import {
  getProfileByUsername, listPublicLinksByUser, incrementClick, getTheme,
      ...
      <div className="mx-auto flex min-h-screen max-w-md flex-col items-center px-5 pb-16 pt-14">
        <div className="flex flex-col items-center text-center">
          {profile.avatar_url ? (
            ...
            <img src={profile.avatar_url} alt={profile.display_name || profile.username} className="h-28 w-28 rounded-full object-cover shadow-lg ring-2 ring-white/30" />
          ) : (
          ...
          <h1 className="mt-5 text-2xl font-bold tracking-tight">{profile.display_name || profile.username}</h1>
        ...
        <footer className="mt-auto pt-16 text-xs opacity-60">
          <a href="/links" className="hover:opacity-100">Make your own →</a>
        </footer>
      </div>
    </div>
  );
